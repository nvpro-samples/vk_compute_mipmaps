...
// a 12x12 tile of the last (2nd) output mip level, generating up to
// 25x25 samples of the intermediate (1st) output mip level along the way.
//
// Dispatch with y, z = 1
...
// needed to generate the 14x14 tile of the second level generated.
shared NVPRO_PYRAMID_SHARED_TYPE sharedLevel_[25][25]; // [y][x]
                            ...
                            bool  boundsCheck_)
{
  ...
  for (int i_ = 0; i_ < iterations_; ++i_)
  {
    ...
    NVPRO_PYRAMID_TYPE sample_ =
        reduceStoreSample_(srcCoord_, srcLevel_, false, kernelSize_,
...
// in shared memory. We need somewhere from 24x24 to 25x25 samples, depending
...
{
  int localIdx_ = int(gl_LocalInvocationIndex);
  ...
  {
    initSharedCoord_ = ivec2(localIdx_ % 25, localIdx_ / 25);
    step_            = ivec2(0, 10);

    if (futureKernelSize_.y == 3)
    {
      // Need the full 25x25 tile; do it as two 25x10 steps and one 25x5 step.
      iterations_ =
          initSharedCoord_.y >= 10 ? 0 : initSharedCoord_.y < 5 ? 3 : 2;
    }
    else  // Future 3x{2,1} kernel
    {
      // Need 25x24 tile; do it as two 25x10 steps and one 25x4 step.
      iterations_ =
          initSharedCoord_.y >= 10 ? 0 : initSharedCoord_.y < 4 ? 3 : 2;
    }
  }
  else
  {
    if (futureKernelSize_.y == 3)
    {
      // Need a 24x25 tile; do as two 10x25 steps and one 4x25 step.
      initSharedCoord_ = ivec2(localIdx_ / 25, localIdx_ % 25);
      step_            = ivec2(10, 0);
      iterations_ =
          initSharedCoord_.x >= 10 ? 0 : initSharedCoord_.x < 4 ? 3 : 2;
    }
    else
    {
      // Need a 24x24 tile; do as two 10x24 steps and one 4x24 step.
      initSharedCoord_ = ivec2(localIdx_ / 24, localIdx_ % 24);
      step_            = ivec2(10, 0);
      iterations_ =
          initSharedCoord_.x >= 10 ? 0 : initSharedCoord_.x < 4 ? 3 : 2;
    }
  ...
  intermediateLevelLoop_(dstTileCoord_ + initSharedCoord_, initSharedCoord_,
...
void nvproPyramidMain()
{
  ...
  {
    // Assign a 12x12 tile of mip level inputLevel_ + 2 to this workgroup.
    int   level2_     = inputLevel_ + 2;
    ivec2 level2Size_ = NVPRO_PYRAMID_LEVEL_SIZE(level2_);
    ivec2 tileCount_  = ivec2(uvec2(level2Size_ + ivec2(11)) / 12u);
    ivec2 tileIdx_    = ivec2(gl_WorkGroupID.x % tileCount_.x,
                              gl_WorkGroupID.x / tileCount_.x);
    ...
    if (boundsCheck_)
    {
      ...
      // compute the above 12x12 tile.
      fillIntermediateTile_(tileIdx_ * 24, true);

      // Compute the inputLevel_ + 2 tile of size 12x12, loading
      // inupts from shared memory.
      if (localIdx_ < 12u * 12u)
      {
        ivec2 threadOffset_ = ivec2(localIdx_ % 12u, localIdx_ / 12u);
        ivec2 dstCoord_     = 12 * tileIdx_ + threadOffset_;
        bool  inBounds_     = uint(dstCoord_.x) < uint(level2Size_.x)
                         && uint(dstCoord_.y) < uint(level2Size_.y);
        ivec2 kernelSize_ =
            kernelSizeFromInputSize_(NVPRO_PYRAMID_LEVEL_SIZE(inputLevel_ + 1));
        if (inBounds_)
        {
          ivec2 sharedCoord_ = 2 * threadOffset_;
          reduceStoreSample_(sharedCoord_, 0, true, kernelSize_,
                             level2Size_, dstCoord_, level2_);
        }
      }
    }
    else
    {
      ...
      fillIntermediateTile_(tileIdx_ * 24, false);
