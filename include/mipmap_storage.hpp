...
public:
  MipmapStorage(uint32_t width, uint32_t height)
  {
    ...
    uint64_t offset = 0;
    m_widthHeight.emplace_back(width, height);
    ...
    while(width != 1 || height != 1)
    {
      ...
      // previous level.
      offset += static_cast<uint64_t>(width) * static_cast<uint64_t>(height);
  ...
  Texel& operator[](glm::uvec3 coord)
  {
    ...
    const auto& dim = m_widthHeight[level];
    assert(x < dim.x && y < dim.y);
    return m_data
        [m_levelOffsets[level] + dim.x * static_cast<uint64_t>(y)
  ...
  size_t getByteSize() const { return sizeof(Texel) * m_data.size(); }
      ...
      uint32_t*   outChannel    = nullptr) const
  {
    ...
    if(outCoordinate)
      *outCoordinate = worstCoordinate;
    if(outChannel)
      *outChannel = worstChannel;
    return worstDelta;
  }
  ...
  void generateMipmaps(ToLinear&& toLinear, FromLinear&& fromLinear)
  {
    ...
    for(uint32_t level = 1; level < m_levelOffsets.size(); ++level)
    {
      const auto& srcDim        = m_widthHeight[level - 1];
      const bool  srcWidthEven  = !(srcDim.x & 1);
      const bool  srcHeightEven = !(srcDim.y & 1);
...
private:
  ...
  void generateLevel(ToLinear&& toLinear, FromLinear&& fromLinear, uint32_t level)
  {
    ...
    const auto& srcDim = m_widthHeight[level - 1U];
    const auto& dstDim = m_widthHeight[level];
    assert(SrcWidthEven == !(srcDim.x & 1));
    assert(SrcHeightEven == !(srcDim.y & 1));
    assert(srcDim.x > 1 || srcDim.y > 1);
    ...
    for(uint32_t y = 0; y < dstDim.y; ++y)
    {
      for(uint32_t x = 0; x < dstDim.x; ++x)
      {
        ...
        auto loadSample = [&](uint32_t xOffset, uint32_t yOffset,
                              Sample& sample) {
          const Texel& texel =
              pSrcLevel[(2 * x + xOffset) + srcDim.x * (2 * y + yOffset)];
        ...
        else
        {
          ...
          // Page 4.
          const float n   = static_cast<float>(dstDim.y);
          const float rcp = 1.0F / (2 * n + 1);
          const float w0  = rcp * (n - static_cast<float>(y));
          const float w1  = rcp * n;
          const float w2  = rcp * static_cast<float>(1 + y);
        ...
        {
          const float n   = static_cast<float>(dstDim.x);
          const float rcp = 1.0F / (2 * n + 1);
          const float w0  = rcp * (n - static_cast<float>(x));
          const float w1  = rcp * n;
          const float w2  = rcp * static_cast<float>(1 + x);
...
inline void cpuGenerateMipmaps_sRGBA(MipmapStorage<uint8_t, 4>* pMips)
{
  auto toLinear = [](std::array<uint8_t, 4> texel) -> std::array<float, 4> {
    return {
        linearFromSrgb(texel[0]), linearFromSrgb(texel[1]),
        linearFromSrgb(texel[2]), texel[3] * (1.F / 255.F)};
  };
  auto fromLinear = [](std::array<float, 4> linear) -> std::array<uint8_t, 4> {
    const uint8_t alpha =
        static_cast<uint8_t>(glm::clamp(linear[3] * 255.F, 0.F, 255.F));
    return {
        static_cast<uint8_t>(srgbFromLinear(linear[0])),
        static_cast<uint8_t>(srgbFromLinear(linear[1])),
        static_cast<uint8_t>(srgbFromLinear(linear[2])),  //
        alpha};
  };
...
inline std::string testMipmaps(const MipmapStorage<uint8_t, 4>& input)
{
  ...
  uint8_t worstDelta = input.compare(expected, &worstCoordinate, &worstChannel);

  return "Worst delta=" + std::to_string(worstDelta) + " at texel ("
         + std::to_string(worstCoordinate.x) + ", "
    ...
    const char*                      pBaseFilename)
{
  ...
  auto threadFunction = [&](uint32_t level) {
    const std::string filename    = level == 0 ?
                                        std::string(pBaseFilename) :
                                        prefix + std::to_string(level) + suffix;
    const auto&       widthHeight = widthHeights[level];
  ...
  const uint32_t parallelLevelCount = levelCount > 8U ? levelCount - 8U : 0U;
